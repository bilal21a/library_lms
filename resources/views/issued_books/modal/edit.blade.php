<div class="fv-row mb-5 fv-plugins-icon-container">
    ...
    <select id="inputState" name="book_name" class="form-select">
        ...
        @foreach ($books as $book)
            <option value="{{ $book->id }}"{{ $data->book_id == $book->id ? 'selected' : '' }}>{{ $book->name }}</option>
        @endforeach
    </select>
...
<div class="fv-row mb-5 fv-plugins-icon-container">
    ...
    <input type="date" class="form-control" name="issue_date" id="datePickerBasic" value="{{$data->issued_date}}"/>
    <div class="fv-plugins-message-container invalid-feedback"></div>
    @error('name')
        <span class="text-danger">{{ $message }}</span>
    @enderror
...
<div class="fv-row mb-5 fv-plugins-icon-container">
    ...
    <input type="date" class="form-control" name="return_date" id="datePickerBasic"  value="{{$data->return_date}}"/>
    <div class="fv-plugins-message-container invalid-feedback"></div>
    @error('name')
        <span class="text-danger">{{ $message }}</span>
    @enderror
